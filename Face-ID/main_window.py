...
# ==============================================================================
# DEBUG = True:  Môi trường development -> Hiển thị tùy chọn chọn camera (nhiều webcam) và thông tin gỡ lỗi.
# DEBUG = False: Sản phẩm cuối (production) -> Ẩn tùy chọn chọn camera, tự động chọn camera mặc định (0).
# Có thể ghi đè qua biến môi trường: set APP_DEBUG=false hoặc set APP_DEBUG=true
DEBUG: bool = os.getenv("APP_DEBUG", "True").lower() in ("true", "1", "yes")


class ManHinhBatDauCa(QWidget):
    def __init__(self, chuyen_man_hinh, debug: bool = DEBUG):
        super().__init__()
        ...
        # Giao diện chọn camera cho máy có nhiều camera
        # CHỈ HIỂN THỊ KHI ĐANG Ở MÔI TRƯỜNG DEVELOPMENT (debug == True)
        self.group_camera = QGroupBox("⚙️ Cấu hình Camera (Môi trường Development)")
        layout_camera = QVBoxLayout(self.group_camera)
        layout_camera.addWidget(QLabel("Chọn thiết bị Camera:"))
        self.combo_camera = QComboBox()
        layout_camera.addWidget(self.combo_camera)
        layout.addWidget(self.group_camera)

        # Ẩn hoặc hiện theo cờ DEBUG
        self.group_camera.setVisible(self.debug)
        if self.debug:
    ...
    def tai_danh_sach_camera(self):
        """Tải danh sách camera khả dụng vào dropdown khi ở chế độ DEBUG."""
    ...
    def bat_dau(self):
        ...
        if ca_hoc_id is not None:
            # Nếu ở môi trường dev thì lấy camera được chọn, ngược lại mặc định camera 0
            camera_id = 0
            if self.debug and self.combo_camera.currentData() is not None:
                camera_id = self.combo_camera.currentData()
            self.chuyen_man_hinh("diem_danh_live", ca_hoc_id, camera_id)
...
class ManHinhDiemDanhLive(QWidget):
    ...
    def vao_man_hinh(self, ca_hoc_id: int, camera_id: int = 0):
        ...
        self.worker = CameraWorker(ca_hoc_id, api_client.goi_diem_danh, camera_id=self.camera_id)
...
class CuaSoChinh(QMainWindow):
    def __init__(self, debug: bool = DEBUG):
        ...
        self.man_bat_dau = ManHinhBatDauCa(self.chuyen_man_hinh, debug=self.debug)
    ...
    def chuyen_man_hinh(self, ten_man_hinh: str, ca_hoc_id, camera_id: int = 0):
        if ten_man_hinh == "diem_danh_live":
            self.man_diem_danh.vao_man_hinh(ca_hoc_id, camera_id)
